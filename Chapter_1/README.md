...
### 1.4.14
**4-sum**  
3-sum中加个循环就好了，见[FourSum.cpp](./FourSum.cpp)。
### 1.4.18
**数组的局部最小元素**  
首先，在题目给定的含有N个不同元素的数组的条件下，`O(lgn)`的解法是存在的，但是如果不是N个不同的元素，则不存在`O(lgn)`的解法。然后，每个数组一定是存在一个局部最小元素的，因为全局最小的元素一定是局部最小的元素。我们有如下的结论：如果数组`a[x,...,y]`，`a[x] <  a[x-1]`, `a[y] < a[y+1]`则在数组`a[x,...,y]`中一定存在一个局部最小值。而题目只要求我们找到一个局部最小的元素，那么可以考虑二分法。  
具体说来：
* 检查数组的中间值a[N/2]以及和它相邻的元素a[N/2-1]和a[N/2+1]
* 如果a[N/2]是一个局部最小值则终止算法，否则继续在较小的相邻元素的半边中继续查找（这样能保证查找的半边中一定有局部最小值）。  

见[LocalMinima.hpp](./LocalMinima.hpp)。
### 1.4.19
**矩阵的局部最小元素**  
跟上一题类似。我们可以先在矩阵中找到中间一行的最小项，假设为`a[N/2][j]`,则这个最小项的两个垂直的邻居为`a[N/2-1][j]`和`a[N/2+1][j]`,如果垂直的两个邻居都比中间一行的最小项大，则找到了局部最小值，否则继续在两个垂直的邻居中较小的那个所在的部分中继续查找。
### 1.4.20
**双调查找**  
由于数组不是有序的，所以不能直接使用二分查找。但是可以先找到最大值然后在最大值的左右两边分别进行二分查找。查找最大值的过程也可以使用二分查找的思想。具体见[BitonicSearch.hpp](BitonicSearch.hpp)。
### 1.4.24
**扔鸡蛋**  
(1). `lgN`。直接使用二分查找的方法即可；  
(2). `lgF`。首先按照1、2、4...2^k的顺序直到鸡蛋破碎，然后在(2^(k-1), 2^k)之间进行二分查找。
### 1.4.25
**扔两个鸡蛋**  
为了得到$2\sqrt{N}$的结果，我们可以按照$\sqrt{N}$, $2\sqrt{N}$, $3\sqrt{N}$...的顺序扔鸡蛋，如果在某一次破了，在用线性查找的方法扔第二枚鸡蛋。
